...
def _slugify(text: str) -> str:
    ...
    Must stay in sync with the identical function in BowlofData_website/build.py
    so that the anchor IDs generated by the site builder match the URLs written
    into summaries_*.json and news_*.md by the newsletter pipeline.
    """
...
class NewsletterPipeline:
    ...
    # ------------------------------------------------------------------
    # Stage 3b — Topic deduplication (dedicated function)
    # ------------------------------------------------------------------

    def stage_dedup_topics(
        self, enriched: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        Detect articles that cover the same topic and keep only the most
        relevant one.

        Two articles are treated as same-topic when they satisfy either:
          • they share ≥ MIN_SHARED_TECHS technologies AND their main_topic
            text has a Jaccard word-overlap of ≥ MIN_TOPIC_JACCARD, or
          • they share ≥ HIGH_TECH_OVERLAP technologies (strong signal alone).

        Within each duplicate group the article is ranked by:
          (quality_score, number of key_points, length of long_resume)
        and only the top-ranked article is kept.
        """
        if len(enriched) <= 1:
            return enriched
        ...
        # Generic tech terms that carry no topic-specificity
        GENERIC_TERMS = {
            "ai", "python", "javascript", "linux", "windows", "macos",
            "cloud", "api", "docker", "kubernetes", "github", "git",
            "machine learning", "deep learning", "llm", "gpt",
        }

        def _tech_set(article: Dict) -> set:
            return {
                t.lower().strip()
                for t in article.get("technologies", [])
                if t and t.lower().strip() not in GENERIC_TERMS
            }
        ...
        def _topic_words(article: Dict) -> set:
            ...
            return set(re.findall(r"\b[a-z]{4,}\b", text)) - GENERIC_TERMS
        ...
        def _is_duplicate(a: Dict, b: Dict) -> bool:
            techs_a = _tech_set(a)
            techs_b = _tech_set(b)
            shared = len(techs_a & techs_b)
            if shared >= HIGH_TECH_OVERLAP:
                return True
            if shared >= MIN_SHARED_TECHS:
                return _jaccard(_topic_words(a), _topic_words(b)) >= MIN_TOPIC_JACCARD
            # Fallback: pure text similarity when tech metadata is sparse
            if not techs_a and not techs_b:
                return _jaccard(_topic_words(a), _topic_words(b)) >= 0.30
            return False

        def _article_score(article: Dict) -> tuple:
            return (
                int(article.get("quality_score", 0)),
                len(article.get("key_points", [])),
                len(article.get("long_resume", "")),
            )
        ...
        for i in range(n):
            for j in range(i + 1, n):
                if _is_duplicate(enriched[i], enriched[j]):
                    _union(i, j)
        ...
        for indices in groups.values():
            if len(indices) == 1:
                result.append(enriched[indices[0]])
                continue
            best_idx = max(indices, key=lambda i: _article_score(enriched[i]))
            best = enriched[best_idx]
            dropped = [enriched[i] for i in indices if i != best_idx]
            logger.info(
                "Topic dedup: keeping '%s' (score=%s), dropping %d duplicate(s): %s",
                best.get("title", "?"),
                best.get("quality_score"),
                len(dropped),
                ", ".join(f"'{a.get('title', '?')}'" for a in dropped),
            )
            result.append(best)
        ...
        if removed:
            logger.info(
                "Stage 3b complete: removed %d same-topic duplicate(s), %d articles remain",
                removed, len(result),
            )
        else:
            logger.info("Stage 3b complete: no same-topic duplicates found")
        return result
    ...
    def stage_write(
        ...
        for rank, article in enumerate(summaries, start=1):
            ...
            url = article.get("url", "")
            link_url = article.get("netlify_url") or article.get("altervista_url") or url
    ...
    def run(self) -> str:
        ...
        enriched = self.stage_dedup_topics(enriched)
    ...
    def run_generate(self) -> str:
        ...
        # Backfill Netlify anchor URLs (requires NETLIFY_SITE_URL in .env)
        all_summaries = self._backfill_netlify_urls(all_summaries)

        # Persist the final state (pruned + re-summarised + netlify_url) so
        # subsequent runs and the website builder see the populated URLs.
        with open(json_path, "w", encoding="utf-8") as fh:
    ...
    def _backfill_netlify_urls(
        ...
        """
        Compute and set ``netlify_url`` on every article in *summaries*.

        The URL format is:
            {NETLIFY_SITE_URL}/week/{WW}_{YYYY}.html#{title-slug}

        The slug is produced by ``_slugify(title)`` which must stay identical to
        the ``_slugify`` function in ``BowlofData_website/build.py`` so that the
        anchor ``id`` on the rendered page matches the URL stored here.

        Does nothing when ``NETLIFY_SITE_URL`` is not configured.
        """
        if not NETLIFY_SITE_URL:
            logger.info("NETLIFY_SITE_URL not set — skipping Netlify anchor URL backfill")
            return summaries

        week_num = getattr(self, "_week_num", None)
        year = getattr(self, "_year", None)
        if not week_num or not year:
            logger.warning("Week/year not set on pipeline — cannot compute Netlify URLs")
            return summaries
        ...
        for article in summaries:
            ...
            if title:
                article["netlify_url"] = f"{page_url}#{_slugify(title)}"
                count += 1
